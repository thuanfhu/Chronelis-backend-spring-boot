...
public class JwtTokenBlacklistFilter extends OncePerRequestFilter {
	...
	protected boolean shouldNotFilter(HttpServletRequest request) {
		...
		return Arrays.stream(SecurityConfiguration.PUBLIC_ENDPOINTS).anyMatch(endpoint -> {
			String pattern = endpoint.replace("{userId}", "[^/]+").replace("{hubId}", "[^/]+")
					.replace("{categoryId}", "[^/]+").replace("{productId}", "[^/]+")
					.replace("{inventoryItemId}", "[^/]+").replace("{slug}", "[^/]+").replace("{code}", "[^/]+")
					.replace("{reviewId}", "[^/]+").replace("{policyId}", "[^/]+").replace("**", ".*");
