...
public class DatabaseSeeder implements ApplicationRunner {
    ...
    static final Pattern VN_PHONE_PATTERN = Pattern
            .compile("^(\\+84|0)(3[2-9]|5[689]|7[06-9]|8[1-9]|9[0-9])[0-9]{7}$");

    static final Set<String> RESERVED_INIT_EMAILS = Set.of(
            "chronelis.admin@gmail.com",
            "chronelis.user@gmail.com",
            "thuanmobile1111@gmail.com");
    ...
    static final String[] FIRST_NAMES = {
            "An", "Bao", "Binh", "Chi", "Dung", "Giang", "Ha", "Hai", "Hanh", "Hieu",
            "Hoa", "Huong", "Huy", "Khanh", "Kiet", "Lam", "Lan", "Linh", "Long", "Mai",
            "Minh", "Nam", "Nga", "Ngoc", "Nhi", "Phong", "Phuc", "Phuong", "Quang", "Son",
            "Thao", "Trang", "Tri", "Tuan", "Van", "Vy", "Yen", "Aiden", "Alex", "Amelia",
            "Benjamin", "Charlotte", "Daniel", "Elena", "Ethan", "Gabriel", "Hannah", "Isabella",
            "Jacob", "James", "Liam", "Lucas", "Mia", "Natalie", "Noah", "Olivia", "Sofia",
            "Thomas", "William", "Yuki", "Diego", "Mateo", "Nora", "Ava", "Leo"
    };

    static final String[] LAST_NAMES = {
            "Nguyen", "Tran", "Le", "Pham", "Hoang", "Phan", "Vu", "Dang", "Bui", "Do",
            "Ngo", "Duong", "Ly", "Truong", "Dinh", "Huynh", "Vo", "Ho", "Cao", "Luu",
            "Smith", "Johnson", "Davis", "Garcia", "Brown", "Miller", "Wilson", "Martin",
            "Anderson", "Thomas", "Lopez", "Hernandez", "Kim", "Lee", "Taylor"
    };

    static final String[] CITIES = {
            "Ho Chi Minh", "Ha Noi", "Da Nang", "Can Tho", "Nha Trang", "Hai Phong", "Hue",
            "Singapore", "Bangkok", "Jakarta", "Tokyo", "Seoul"
    };
    ...
    static final String[] TEAM_NAME_POOL = {
            "Backend Team", "Frontend Team", "Mobile Team", "Product Team", "QA Team", "Design Team",
            "Growth Team", "Operations Team", "Data Team", "Analytics Team", "DevOps Team",
            "Customer Success Team", "Compliance Team", "Security Team", "Content Team", "Partnership Team"
    };
    ...
    static final List<TaskTypeTemplate> TASK_TYPE_TEMPLATES = List.of(
    ...
    private void seedAll(Random random, LocalDateTime now) {
        ...
        MembershipSeedResult membershipSeed = seedWorkspaceMembers(random, now, workspaces, userSeed.allUsers());
        ...
        List<Notification> notifications = seedNotifications(
                random, now, membershipSeed.memberships(), taskSeed.tasks(), scheduleSeed.schedulesByTaskId(),
                commentSeed.commentsByTaskId(), goalsByProjectId, invites);

        log.info(">>> SEED STEP: activity logs");
        List<ActivityLog> activityLogs = seedActivityLogs(
                random, now, workspaces, membershipSeed, teamSeed, projectsByWorkspaceId, goalsByProjectId,
                taskTypesByProjectId, taskSeed.tasks(), scheduleSeed.schedules(), commentSeed.comments(), invites);
        ...
        double avgWorkspacesPerUser = userSeed.allUsers().isEmpty()
                ? 0D
    ...
    private UserSeedResult seedUsers(Random random) {
        Role userRole = roleRepository.findByName(RoleType.USER_ROLE.getName())
                .orElseThrow(() -> new IllegalStateException("USER role not found. Run DataInitializer first."));
        ...
        for (User user : existingUsers) {
            ...
            String nickname = (firstToken + lastToken).length() > 20
                    ? (firstToken + lastToken).substring(0, 20)
                    : firstToken + lastToken;

            if (user.getNickname() == null || user.getNickname().isBlank() || user.getNickname().trim().length() < 2) {
                user.setNickname(nickname);
                changed = true;
            }

            if (user.getAvatarUrl() == null || user.getAvatarUrl().isBlank()) {
                user.setAvatarUrl("https://api.dicebear.com/7.x/initials/svg?seed=" + firstName + "%20" + lastName);
                changed = true;
            }

            if (user.getBiography() == null || user.getBiography().isBlank()) {
                String focus = BIO_FOCUS[(profileCursor * 5 + 2) % BIO_FOCUS.length];
                user.setBiography("Contributes to " + focus
        ...
        while (generated.size() < usersToCreate) {
            ...
            generated.add(User.builder()
                    ...
                    .biography("Works across " + focus + " and collaborates closely with product and engineering.")
                    .city(city)
                    .nationality("Vietnam")
                    .isVerified(true)
    ...
    private List<Workspace> seedWorkspaces(Random random, LocalDateTime now, List<User> users) {
        ...
        for (int index = 0; index < WORKSPACE_NAMES.length; index++) {
            ...
            LocalDateTime updatedAt = randomDateTimeBetween(random, createdAt.plusDays(20), now.minusDays(1));
    ...
    private MembershipSeedResult seedWorkspaceMembers(
        ...
        for (int workspaceIndex = 0; workspaceIndex < workspaces.size(); workspaceIndex++) {
            ...
            int targetMembers = Math.min(resolveWorkspaceMemberTarget(workspaceIndex, users.size()), users.size());
            ...
            targetMembershipCountByUserId.compute(ownerId,
                    (key, value) -> Math.max(value == null ? USER_MIN_WORKSPACES : value, 1));
        ...
        for (int userIndex = 0; userIndex < orderedUsers.size(); userIndex++) {
            ...
            int preferredWorkspaceIndex = primaryWorkspaceIndexByUserId.getOrDefault(
                    userId,
                    Math.floorMod((userIndex * 9) + random.nextInt(workspaces.size()), workspaces.size()));
            primaryWorkspaceIndexByUserId.putIfAbsent(userId, preferredWorkspaceIndex);

            int targetMembership = targetMembershipCountByUserId.getOrDefault(userId, USER_MIN_WORKSPACES);
            ...
            while (assignedMembership < targetMembership && attempts < workspaces.size() * 4) {
                Integer chosenWorkspaceIndex = chooseWorkspaceForUser(
                        random,
                        user,
                        preferredWorkspaceIndex,
                        workspaceMemberSets,
                        workspaceTargets);
            ...
            if (assignedMembership < targetMembership) {
                for (int offset = 0; offset < workspaces.size() && assignedMembership < targetMembership; offset++) {
        ...
        for (int workspaceIndex = 0; workspaceIndex < workspaces.size(); workspaceIndex++) {
            ...
            nonOwnerMembers.sort(Comparator
                    .comparingInt((User user) -> assignedMembershipCountByUserId
                            .getOrDefault(user.getUserId(), USER_MIN_WORKSPACES))
                    .reversed()
    ...
    private TeamSeedResult seedWorkspaceTeams(
        ...
        for (int workspaceIndex = 0; workspaceIndex < workspaces.size(); workspaceIndex++) {
            Workspace workspace = workspaces.get(workspaceIndex);
            List<User> admins = membershipSeed.adminsByWorkspaceId().getOrDefault(workspace.getId(), List.of());
            List<User> workspaceMembers = membershipSeed.membersByWorkspaceId().getOrDefault(workspace.getId(),
                    List.of());
            ...
            for (int teamIndex = 0; teamIndex < teamNames.size(); teamIndex++) {
                User creator = admins.isEmpty() ? workspace.getOwner()
        ...
        for (WorkspaceTeam team : savedTeams) {
            teamsByWorkspaceId.computeIfAbsent(team.getWorkspace().getId(), key -> new ArrayList<>()).add(team);
        }
    ...
    private List<Project> seedProjects(
        ...
        for (int workspaceIndex = 0; workspaceIndex < workspaces.size(); workspaceIndex++) {
            Workspace workspace = workspaces.get(workspaceIndex);
            List<User> admins = membershipSeed.adminsByWorkspaceId().getOrDefault(workspace.getId(), List.of());
            List<WorkspaceTeam> teams = teamSeed.teamsByWorkspaceId().getOrDefault(workspace.getId(), List.of());
            ...
            for (int projectIndex = 0; projectIndex < projectCount; projectIndex++) {
                ...
                User managerUser = pickManagerUser(random, managerTeam, workspace, membershipSeed, teamSeed);
                User creator = admins.isEmpty() ? workspace.getOwner() : admins.get(random.nextInt(admins.size()));
    ...
    private Integer chooseWorkspaceForUser(
        ...
        candidates.sort(Comparator.comparingDouble(workspaceIndex -> workspaceSelectionScore(
    ...
    private String buildProjectName(Workspace workspace, int workspaceIndex, int projectIndex) {
        String baseName = PROJECT_NAME_POOL[(workspaceIndex * 5 + projectIndex) % PROJECT_NAME_POOL.length];
    ...
    private Map<Long, List<TaskStatus>> seedTaskStatuses(List<Project> projects) {
        ...
        for (Project project : projects) {
            statuses.add(TaskStatus.builder()
                    .project(project)
                    .name("Backlog")
                    .code("BACKLOG")
                    .position(0)
    ...
    private Map<Long, List<Goal>> seedGoals(
        ...
        for (Project project : projects) {
            ...
            List<WorkspaceTeam> workspaceTeams = teamSeed.teamsByWorkspaceId()
                    .getOrDefault(project.getWorkspace().getId(), List.of());
            ...
            for (int index = 0; index < goalCount; index++) {
                ...
                User managerUser = chooseGoalManagerUser(
                        random, project, managerTeam, workspaceMembers, membershipSeed, teamSeed);
                User creator = workspaceMembers.isEmpty()
    ...
    private Map<Long, List<TaskType>> seedTaskTypes(
        ...
        for (Project project : projects) {
            ...
            for (TaskTypeTemplate template : TASK_TYPE_TEMPLATES) {
                if ("Operations".equals(template.name()) && project.getStatus() == ProjectStatusType.ARCHIVED) {
                    continue;
                }
                ...
                Goal linkedGoal = goals.isEmpty() || random.nextDouble() >= 0.18
                        ? null
                ...
                LocalDateTime updatedAt = randomDateTimeBetween(
                        random,
                        createdAt,
                        minDateTime(now.minusDays(1), createdAt.plusDays(25)));

                taskTypes.add(TaskType.builder()
        ...
        for (TaskType taskType : savedTaskTypes) {
            grouped.computeIfAbsent(taskType.getProject().getId(), key -> new ArrayList<>()).add(taskType);
        }
        return grouped;
    }
    ...
    private TaskSeedResult seedTasks(
        ...
        for (Task task : savedTasks) {
            tasksByProjectId.computeIfAbsent(task.getProject().getId(), key -> new ArrayList<>()).add(task);
        }
    ...
    private Task buildTaskEntity(
        ...
        List<User> collaborationCandidates = buildCollaborationCandidates(collaborationOrder, creatorIndex, 7);
        ...
        ImportanceLevel importance = priority == TaskPriorityType.LOW && random.nextDouble() < 0.7
                ? ImportanceLevel.LOW
                : ImportanceLevel.HIGH;
        UrgencyLevel urgency = (priority == TaskPriorityType.URGENT || priority == TaskPriorityType.HIGH)
                ? UrgencyLevel.HIGH
        ...
        String description = random.nextDouble() < 0.82
                ? buildTaskDescription(title, project, goal)
                : null;
        String notesHtml = random.nextDouble() < 0.35
                ? buildTaskNotesHtml(project, random)
                : null;
    ...
    private ScheduleSeedResult seedTaskSchedules(Random random, LocalDateTime now, List<Task> tasks) {
        ...
        int dayWindow = 8; // today + next 7 days

        Map<Integer, List<LocalDateTime>> laneCursorsByDayOffset = new HashMap<>();
        for (int dayOffset = 0; dayOffset < dayWindow; dayOffset++) {
            LocalDateTime dayBaseStart = anchorDate.plusDays(dayOffset).atTime(8, 30);
            if (dayOffset == 0) {
                dayBaseStart = maxDateTime(dayBaseStart, nowAnchor);
            }

            int laneCount = dayOffset <= 2 ? 4 : 3;
            List<LocalDateTime> laneCursors = new ArrayList<>(laneCount);
            for (int lane = 0; lane < laneCount; lane++) {
                laneCursors.add(ceilToQuarterHour(dayBaseStart.plusMinutes(lane * 20L)));
            }
            laneCursorsByDayOffset.put(dayOffset, laneCursors);
        }

        int seededTaskPointer = 0;
        ...
        for (Task task : tasks) {
            ...
            double baseProbability = switch (task.getSourceView()) {
                case CALENDAR -> 0.88;
                case TODO -> 0.66;
                case KANBAN -> 0.38;
            };
            ...
            if (task.getProject().getStatus() == ProjectStatusType.ACTIVE) {
                baseProbability += 0.06;
            }
            if (random.nextDouble() > Math.min(baseProbability, 0.95)) {
                continue;
            }

            int scheduleCount = random.nextDouble() < 0.22 ? 2 : 1;
            int primaryDayOffset = seededTaskPointer < 96
                    ? seededTaskPointer % dayWindow
                    : pickNearTermDayOffset(random);
            seededTaskPointer++;

            for (int index = 0; index < scheduleCount; index++) {
                int preferredDayOffset = Math.min(dayWindow - 1,
                        primaryDayOffset + (index * (1 + random.nextInt(2))));
                ReservedSlot reservedSlot = reserveNearTermSlot(random, laneCursorsByDayOffset,
                        preferredDayOffset, nowAnchor, task.getPriority());
                if (reservedSlot == null) {
                    continue;
                }

                LocalDateTime start = reservedSlot.start();
                LocalDateTime end = reservedSlot.end();
                User creator = task.getAssignee() != null ? task.getAssignee() : task.getCreatedBy();

                LocalDateTime createdLowerBound = maxDateTime(task.getCreatedAt().plusMinutes(10),
                        now.minusDays(21));
                LocalDateTime createdUpperBound = minDateTime(now.minusMinutes(1), start.minusMinutes(5));

                LocalDateTime createdAt;
                if (createdUpperBound != null && createdLowerBound != null
                        && createdUpperBound.isAfter(createdLowerBound)) {
                    createdAt = randomDateTimeBetween(random, createdLowerBound, createdUpperBound);
                } else {
                    createdAt = minDateTime(now.minusMinutes(1), maxDateTime(task.getCreatedAt(), now.minusDays(1)));
                }
        ...
        for (TaskSchedule schedule : savedSchedules) {
            schedulesByTaskId.computeIfAbsent(schedule.getTask().getId(), key -> new ArrayList<>()).add(schedule);
        }
    ...
    private long resolveScheduleDurationMinutes(Random random, TaskPriorityType priority) {
        int[] durationOptions = switch (priority) {
    ...
    private CommentSeedResult seedTaskComments(
        ...
        for (Task task : tasks) {
            ...
            for (int index = 0; index < commentCount; index++) {
                ...
                LocalDateTime updatedAt = random.nextDouble() < 0.22
                ...
                String content = COMMENT_TEMPLATES[(index + random.nextInt(COMMENT_TEMPLATES.length))
                        % COMMENT_TEMPLATES.length];
                topLevelComments.add(TaskComment.builder()
                        .task(task)
                        .user(author)
                        .parentComment(null)
        ...
        for (TaskComment parent : savedTopLevel) {
            ...
            LocalDateTime updatedAt = random.nextDouble() < 0.15
                    ? createdAt.plusMinutes(10 + random.nextInt(60))
                    : createdAt;
            if (updatedAt.isAfter(now.minusMinutes(1))) {
                updatedAt = now.minusMinutes(1);
            }
            ...
            replies.add(TaskComment.builder()
                    .task(parent.getTask())
                    .user(replier)
        ...
        for (TaskComment comment : allComments) {
            commentsByTaskId.computeIfAbsent(comment.getTask().getId(), key -> new ArrayList<>()).add(comment);
        }
    ...
    private List<WorkspaceInvite> seedWorkspaceInvites(
        ...
        for (Workspace workspace : workspaces) {
            ...
            LocalDateTime createdBase = minDateTime(now.minusDays(20), workspace.getCreatedAt().plusDays(20));
    ...
    private List<Notification> seedNotifications(
        ...
        for (WorkspaceMember member : memberships) {
            ...
            addNotification(
                    ...
                    "You were added to workspace " + member.getWorkspace().getName() + " as " + member.getRole() + ".",
        ...
        for (List<Goal> goals : goalsByProjectId.values()) {
            for (Goal goal : goals) {
                ...
                User recipient = goal.getManagerUser() != null ? goal.getManagerUser() : goal.getCreatedBy();
                addNotification(
                        notifications,
                        random,
                        now,
        ...
        for (WorkspaceInvite invite : invites) {
            ...
            addNotification(
                    ...
                    "An invite for workspace " + invite.getWorkspace().getName() + " was used by a new member.",
    ...
    private List<ActivityLog> seedActivityLogs(
        ...
        for (WorkspaceMember member : membershipSeed.memberships()) {
            ...
            List<User> admins = membershipSeed.adminsByWorkspaceId().getOrDefault(workspace.getId(), List.of());
            User actor = admins.isEmpty() ? workspace.getOwner() : admins.get(random.nextInt(admins.size()));
        ...
        for (Task task : tasks) {
            ...
            if (task.getAssignee() != null && !task.getAssignee().getUserId().equals(task.getCreatedBy().getUserId())) {
                addActivityLog(
                        logs,
                        now,
                        workspace,
            ...
            if (task.getUpdatedAt().isAfter(task.getCreatedAt().plusHours(8)) && random.nextDouble() < 0.28) {
                addActivityLog(
                        logs,
                        now,
                        workspace,
        ...
        for (WorkspaceInvite invite : invites) {
            ...
            if (invite.getUsedCount() > 0) {
                List<User> members = membershipSeed.membersByWorkspaceId().getOrDefault(workspace.getId(), List.of());
            ...
            if (!Boolean.TRUE.equals(invite.getIsActive())
                ...
                LocalDateTime revokeTime = invite.getExpiresAt() != null
                        ? invite.getExpiresAt()
    ...
    private User pickManagerUser(
            ...
            TeamSeedResult teamSeed) {
        if (managerTeam != null) {
            List<User> teamMembers = teamSeed.membersByTeamId().getOrDefault(managerTeam.getId(), List.of());
    ...
    private GoalStatusType chooseGoalStatus(ProjectStatusType projectStatus, Random random) {
        ...
        return switch (projectStatus) {
            case ACTIVE -> {
                if (roll < 20)
                    yield GoalStatusType.NOT_STARTED;
                if (roll < 72)
                    yield GoalStatusType.IN_PROGRESS;
                if (roll < 87)
                    yield GoalStatusType.COMPLETED;
    ...
    private WorkspaceTeam chooseGoalManagerTeam(Random random, Project project, List<WorkspaceTeam> workspaceTeams) {
    ...
    private User chooseGoalManagerUser(
        ...
        List<User> admins = membershipSeed.adminsByWorkspaceId()
                .getOrDefault(project.getWorkspace().getId(), List.of());
        if (!admins.isEmpty()) {
            return admins.get(random.nextInt(admins.size()));
        }
    ...
    private int resolveTaskCount(Project project, Random random) {
        int base = switch (project.getStatus()) {
    ...
    private TaskStatus chooseTaskStatus(Random random, ProjectStatusType projectStatus, List<TaskStatus> statuses) {
        int roll = random.nextInt(100);
        return switch (projectStatus) {
            case ACTIVE -> {
                if (roll < 18)
    ...
    private SourceViewType chooseSourceView(Random random) {
        int roll = random.nextInt(100);
        if (roll < 64)
            return SourceViewType.KANBAN;
        if (roll < 85)
            return SourceViewType.TODO;
    ...
    private LocalDateTime resolveTaskDueDate(
        ...
        boolean overdue = random.nextDouble() < (projectStatus == ProjectStatusType.ACTIVE ? 0.24 : 0.18);
        if (overdue) {
    ...
    private int resolveCommentCount(Random random, Task task) {
        ...
        if (task.getPriority() == TaskPriorityType.URGENT || task.getPriority() == TaskPriorityType.HIGH) {
            min = 4;
        }
    ...
    private List<User> buildCommentParticipants(Random random, Task task, List<User> workspaceMembers) {
    ...
    private List<User> buildCollaborationCandidates(List<User> orderedMembers, int anchorIndex, int maxCandidates) {
    ...
    private User pickDifferentUser(Random random, List<User> candidates, User excluded) {
        ...
        for (User candidate : candidates) {
            if (candidate != null && (excluded == null || !candidate.getUserId().equals(excluded.getUserId()))) {
    ...
    private record ScheduleSeedResult(List<TaskSchedule> schedules, Map<Long, List<TaskSchedule>> schedulesByTaskId) {
    }

    private record CommentSeedResult(List<TaskComment> comments, Map<Long, List<TaskComment>> commentsByTaskId) {
    }
}