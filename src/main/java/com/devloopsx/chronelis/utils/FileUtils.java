...
public class FileUtils {
  ...
  public void validateFile(MultipartFile file) {
    if (file == null || file.isEmpty()) throw new ApplicationException(
      ErrorCode.FILE_NOT_FOUND
    );

    String contentType = resolveContentType(file);
    List<String> normalizedAllowedFileTypes = allowedFileTypes
      .stream()
    ...
    if (
      ...
      !normalizedAllowedFileTypes.contains(contentType)
    ) {
  ...
  public void validateFileSize(long size) {
    ...
    if (size < minFileSize) {
      throw new ApplicationException(
        ...
        "Kich thuoc file qua nho. Dung luong toi thieu phai la " +
          (minFileSize / 1024) +
          " KB"
      );
    }
  ...
  public void validateFilePath(String filePath) {
    ...
    if (
      filePath == null || !filePath.matches(filePathRegex)
