...
public class SecurityUtils {
  ...
  public ResponseEntity<AuthenticationResponse> createAuthResponse(User currentUser) {
    if (Objects.isNull(currentUser)) throw new ApplicationException(ErrorCode.USER_NOT_FOUND);
  ...
  public boolean isAdmin(User user) {
    return user.getRoles() != null
        && user.getRoles().stream()
            .anyMatch(role -> RoleType.ADMIN_ROLE.getName().equals(role.getName()));
  }
        ...
        new JWTClaimsSet.Builder()
            ...
            .expirationTime(
                new Date(
                    Instant.now()
                        .plus(
                            Objects.equals(keyType, TokenType.ACCESS_TOKEN.getKey())
  ...
  public void updateUserRefreshToken(String refreshToken, String email) {
    ...
    for (int attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (affected == 0) throw new ApplicationException(ErrorCode.USER_NOT_FOUND);
        return;
      } catch (org.springframework.dao.PessimisticLockingFailureException ex) {
        if (attempt == 3) {
          log.warn(
              "Could not update refresh token for user {} after 3 attempts, continuing login anyway",
              email);
          return; // graceful degradation — login still succeeds
        }
        log.warn(
            "Lock contention on refresh token update for user {}, retrying (attempt {}/3)...",
            email,
            attempt);
      }
    }
  ...
  public void enforceProtectedEmailPolicy(String email) {
    List<String> protectedEmails =
        Arrays.asList(
            "chronelis.admin@gmail.com",
            "chronelis.customer@gmail.com",
  ...
  public void validateOwnership(String ownerId) {
    User currentUser =
        userRepository
            .findByEmail(getCurrentUserEmailFromJwt())
            .orElseThrow(() -> new ApplicationException(ErrorCode.USER_NOT_FOUND));
