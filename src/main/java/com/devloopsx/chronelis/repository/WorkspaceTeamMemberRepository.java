...
public interface WorkspaceTeamMemberRepository extends JpaRepository<WorkspaceTeamMember, Long> {
  ...
  @Query(
      "SELECT wtm.team.id FROM WorkspaceTeamMember wtm WHERE wtm.team.workspace.id = :workspaceId AND wtm.user.userId = :userId")
  List<Long> findTeamIdsByWorkspaceIdAndUserId(
