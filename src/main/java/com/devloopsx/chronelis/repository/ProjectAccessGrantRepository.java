...
public interface ProjectAccessGrantRepository extends JpaRepository<ProjectAccessGrant, Long> {
    ...
    List<ProjectAccessGrant> findByProjectIdAndSubjectTypeAndTeamIdIn(Long projectId, ProjectAccessSubjectType subjectType,
    ...
    @Query("SELECT DISTINCT pa.project.id FROM ProjectAccessGrant pa WHERE pa.subjectType = com.devloopsx.chronelis.constant.ProjectAccessSubjectType.USER AND pa.user.userId = :userId")
    List<Long> findProjectIdsByUserGrant(@Param("userId") String userId);

    @Query("SELECT DISTINCT pa.project.id FROM ProjectAccessGrant pa WHERE pa.subjectType = com.devloopsx.chronelis.constant.ProjectAccessSubjectType.TEAM AND pa.team.id IN :teamIds")
