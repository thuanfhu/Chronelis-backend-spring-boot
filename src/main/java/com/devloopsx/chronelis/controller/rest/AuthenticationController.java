...
public class AuthenticationController {
	...
	ApiResponse<Void> register(@RequestBody @Valid RegisterUserRequest registerUserRequest,
		...
		return ApiResponse.<Void>builder().message("Vui lòng kiểm tra email để xác thực tài khoản")
	...
	ApiResponse<AuthenticationResponse> verifyEmailAndActivateAccount(
		...
		return ApiResponse.<AuthenticationResponse>builder().message("Email của bạn đã được xác thực thành công")
	...
	ApiResponse<Void> resendVerifyEmail(@RequestBody @Valid ResendVerifyEmailRequest resendVerifyEmailRequest,
		...
		return ApiResponse.<Void>builder().message("Email xác thực đã được gửi lại. Vui lòng kiểm tra hộp thư đến")
				.meta(buildMetaInfo(servletRequest)).build();
	}
	...
	ResponseEntity<ApiResponse<AuthenticationResponse>> login(@Valid @RequestBody LoginRequest loginRequest,
		...
		ApiResponse<AuthenticationResponse> apiResponse = ApiResponse.<AuthenticationResponse>builder()
				.message("Đăng nhập thành công").data(responseEntity.getBody()).meta(buildMetaInfo(servletRequest))
				.build();
	...
	ResponseEntity<ApiResponse<Void>> logout(HttpServletRequest servletRequest) {
		...
		ApiResponse<Void> apiResponse = ApiResponse.<Void>builder().message("Đăng xuất thành công")
				.meta(buildMetaInfo(servletRequest)).build();
	...
	ApiResponse<UserSecureResponse> getMyInfo(HttpServletRequest servletRequest) {
		return ApiResponse.<UserSecureResponse>builder().message("Lấy thông tin người dùng đã xác thực thành công")
				.data(authenticationService.getMyInfo()).meta(buildMetaInfo(servletRequest)).build();
	}
	...
	ResponseEntity<ApiResponse<AuthenticationResponse>> getNewToken(
		...
		ApiResponse<AuthenticationResponse> apiResponse = ApiResponse.<AuthenticationResponse>builder()
				.message("Lấy thành công refresh token và access token").data(responseEntity.getBody())
				.meta(buildMetaInfo(servletRequest)).build();
	...
	ApiResponse<Void> forgotPassword(@RequestBody @Valid ForgotPasswordRequest forgotPasswordRequest,
		...
		return ApiResponse.<Void>builder().message("Vui lòng kiểm tra email để đặt lại mật khẩu")
				.meta(buildMetaInfo(servletRequest)).build();
	}
