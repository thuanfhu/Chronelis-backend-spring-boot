...
public class RoleController {
	...
	ApiResponse<RoleResponse> createRole(@RequestBody @Valid CreateRoleRequest createRoleRequest,
		...
		return ApiResponse.<RoleResponse>builder().message("Tạo vai trò thành công")
				.data(roleService.createRole(createRoleRequest)).meta(buildMetaInfo(servletRequest)).build();
	}

	@GetMapping("/{roleId}")
	ApiResponse<RoleResponse> getRoleById(@PathVariable String roleId, HttpServletRequest servletRequest) {
		return ApiResponse.<RoleResponse>builder().message("Lấy thông tin vai trò thành công")
				.data(roleService.getRoleById(roleId)).meta(buildMetaInfo(servletRequest)).build();
	}

	@GetMapping
	ApiResponse<PaginationResponse> getAllRoleWithQuery(@Filter Specification<Role> spec, Pageable pageable,
			HttpServletRequest servletRequest) {
		return ApiResponse.<PaginationResponse>builder().message("Lấy danh sách vai trò thành công với bộ lọc truy vấn")
				.data(roleService.getAllRoleWithQuery(spec, pageable)).meta(buildMetaInfo(servletRequest)).build();
	}

	@PatchMapping("/{roleId}")
	ApiResponse<RoleResponse> updateRoleById(@PathVariable String roleId,
			@RequestBody UpdateRoleRequest updateRoleRequest, HttpServletRequest servletRequest) {
		return ApiResponse.<RoleResponse>builder().message("Cập nhật vai trò thành công")
				.data(roleService.updateRoleById(roleId, updateRoleRequest)).meta(buildMetaInfo(servletRequest))
				.build();
	}

	@DeleteMapping("/{roleId}/permissions")
	ApiResponse<Void> deletePermissionFromRole(@PathVariable String roleId,
		...
		return ApiResponse.<Void>builder().message("Xóa quyền khỏi vai trò thành công")
	...
	ApiResponse<Void> deleteRoleById(@PathVariable String roleId, HttpServletRequest servletRequest) {
		...
		return ApiResponse.<Void>builder().message("Xóa vai trò thành công").meta(buildMetaInfo(servletRequest))
				.build();
	}
}
