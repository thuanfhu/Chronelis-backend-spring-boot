...
public class PomodoroSessionController {
    ...
    public ResponseEntity<ApiResponse<PomodoroSessionResponse>> saveSession(
        ...
        return ResponseEntity.ok(ApiResponse.<PomodoroSessionResponse>builder()
    ...
    public ResponseEntity<ApiResponse<List<PomodoroSessionResponse>>> getSessions(
            @PathVariable Long taskId,
            HttpServletRequest servletRequest) {
        List<PomodoroSessionResponse> sessions = pomodoroSessionService.getCurrentUserSessionsByTask(taskId);
        return ResponseEntity.ok(ApiResponse.<List<PomodoroSessionResponse>>builder()
