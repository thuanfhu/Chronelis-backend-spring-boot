...
/**
 * Utility class for mapping validation errors to ErrorDetail objects
 * Supports both REST and WebSocket validation errors
 */
public class ErrorDetailMapper {
	...
	/**
	 * Map validation errors from WebSocket Controller (@ControllerAdvice for
	 * WebSocket)
	 */
	...
	/**
	 * Common method to map validation errors from BindingResult
	 * Reused by both REST and WebSocket error mappers
	 *
	 * @param bindingResult
	 *            The binding result containing validation errors
	 * @return List of ErrorDetail objects
	 */
	private static List<ErrorDetail> mapValidationErrorsFromBindingResult(BindingResult bindingResult) {
		return bindingResult.getAllErrors().stream().map(ErrorDetailMapper::mapSingleError)
	...
	private static ErrorDetail mapSingleError(ObjectError error) {
		...
		return ErrorDetail.builder().resource(resource).field(field).code(errorCode.getCode())
