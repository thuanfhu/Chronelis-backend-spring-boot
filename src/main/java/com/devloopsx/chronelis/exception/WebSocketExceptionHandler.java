 ...
 *
 * <p>Handles exceptions from NON-ASYNC WebSocket message handlers For @Async methods, exceptions
 * must be caught manually in the controller because @MessageExceptionHandler cannot intercept
 * exceptions from async threads
 */
@ControllerAdvice
@Component
...
public class WebSocketExceptionHandler {
  ...
  private ApiResponse<?> buildErrorResponse(
      ErrorCode errorCode, Optional<String> customMessage, List<ErrorDetail> errorDetails) {
    ...
    return ApiResponse.builder().success(false).errors(details).meta(null).build();
  }
  ...
  public ApiResponse<?> handleApplicationException(
      ApplicationException ex, SimpMessageHeaderAccessor headerAccessor) {
  ...
  /**
   * Handle MethodArgumentNotValidException (validation errors from @Valid) This catches validation
   * errors BEFORE the method is invoked
   */
  @MessageExceptionHandler(MethodArgumentNotValidException.class)
  @SendToUser("/private/errors")
  public ApiResponse<?> handleValidationException(
      MethodArgumentNotValidException ex, SimpMessageHeaderAccessor headerAccessor) {
    String userId = getUserId(headerAccessor);
  ...
  @SendToUser("/private/errors")
  public ApiResponse<?> handleMessageConversionException(
      MessageConversionException ex, SimpMessageHeaderAccessor headerAccessor) {
    String userId = getUserId(headerAccessor);
    log.error("WebSocket MessageConversionException for userId={}: {}", userId, ex.getMessage());

    return buildErrorResponse(
        ErrorCode.INVALID_REQUEST_DATA, Optional.of("Định dạng dữ liệu không hợp lệ"), null);
  }
  ...
  public ApiResponse<?> handleAllExceptions(
      Throwable ex, SimpMessageHeaderAccessor headerAccessor) {
    String userId = getUserId(headerAccessor);
    log.error("WebSocket Unhandled error for userId={}: {}", userId, ex.getMessage(), ex);

    return buildErrorResponse(
        ErrorCode.UNCATEGORIZED_EXCEPTION, Optional.of("Đã xảy ra lỗi không mong muốn"), null);
  }

  /** Get authenticated userId from Principal (set by UserInterceptor after JWT validation) */
  ...
  public void sendErrorToUser(String userId, ErrorCode errorCode, String message) {
    ErrorDetail errorDetail =
        ErrorDetail.builder().code(errorCode.getCode()).message(message).build();

    ApiResponse<?> errorResponse =
        ApiResponse.builder().success(false).errors(List.of(errorDetail)).meta(null).build();

    String destination = "/private/errors";
    messagingTemplate.convertAndSendToUser(userId, destination, errorResponse);
    log.info(
        "Sent error to user {} at /user/{}{} - Code: {} - Message: {}",
        userId,
        userId,
        destination,
        errorCode.getCode(),
  ...
  public void sendValidationErrorsToUser(String userId, List<ErrorDetail> errorDetails) {
    ApiResponse<?> errorResponse =
        ApiResponse.builder().success(false).errors(errorDetails).meta(null).build();

    String destination = "/private/errors";
    messagingTemplate.convertAndSendToUser(userId, destination, errorResponse);
    log.info(
        "Sent {} validation errors to user {} at /user/{}{}",
        errorDetails.size(),
        userId,
        userId,
        destination);
