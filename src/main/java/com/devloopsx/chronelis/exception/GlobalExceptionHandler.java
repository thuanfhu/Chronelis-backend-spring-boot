...
public class GlobalExceptionHandler {
	...
	private ResponseEntity<ApiResponse<?>> buildErrorResponse(ErrorCode errorCode, Optional<String> customMessage,
			List<ErrorDetail> errorDetails, HttpServletRequest request) {
		if (errorDetails == null || errorDetails.isEmpty()) {
			ErrorDetail detail = ErrorDetail.builder().code(errorCode.getCode())
					.message(customMessage.orElse(errorCode.getMessage())).build();
			errorDetails = List.of(detail);
		}
		ApiResponse<?> apiResponse = ApiResponse.builder().success(false).errors(errorDetails)
				.meta(buildMetaInfo(request)).build();
	...
	public ResponseEntity<ApiResponse<?>> handleAppException(ApplicationException exception,
			HttpServletRequest request) {
