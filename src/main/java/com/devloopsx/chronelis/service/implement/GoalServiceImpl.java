...
public class GoalServiceImpl implements GoalService {
  ...
  public GoalResponse createGoal(CreateGoalRequest request) {
    ...
    goal.setProgressPercent(
        request.getProgressPercent() == null ? BigDecimal.ZERO : request.getProgressPercent());
    goal.setCreatedAt(now);
    goal.setUpdatedAt(now);

    applyGoalManagerAssignments(
        goal,
    ...
    realtimeEventPublisherService.publishProjectEvent(
        project.getWorkspace().getId(), project.getId(), "goal.created", response);
    return response;
  }
  ...
  public GoalResponse updateGoal(Long goalId, UpdateGoalRequest request) {
    ...
    if (managerUpdateRequested) {
      collaborationAccessService.ensureCurrentUserIsWorkspaceOwner(
          goal.getProject().getWorkspace().getId());
    }

    if (request.getTitle() == null
        && request.getGoalType() == null
  ...
  public void deleteGoal(Long goalId) {
    ...
    activityLogService.createLog(
        workspaceId,
        currentUser.getUserId(),
        ActivityActionType.GOAL_DELETED,
        ActivityTargetType.GOAL,
        goalId,
        "Xóa goal " + goal.getTitle());

    realtimeEventPublisherService.publishProjectEvent(
        workspaceId, projectId, "goal.deleted", goalId);
  }
  ...
  private void validateProgress(BigDecimal progress) {
    ...
    if (progress.compareTo(BigDecimal.ZERO) < 0
      ...
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Tiến độ goal phải nằm trong khoảng 0..100");
    }
  }

  private void applyGoalManagerAssignments(
      Goal goal, Long workspaceId, String managerUserId, Long managerTeamId) {
