...
public class TaskStatusServiceImpl implements TaskStatusService {
  ...
  public TaskStatusResponse createStatus(CreateTaskStatusRequest request) {
    ...
    if (taskStatusRepository.existsByProjectIdAndCodeIgnoreCase(project.getId(), normalizedCode)) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Code status đã tồn tại trong project");
    }

    List<TaskStatus> statuses =
        taskStatusRepository.findByProjectIdOrderByPositionAsc(project.getId());
    ...
    realtimeEventPublisherService.publishProjectEvent(
        project.getWorkspace().getId(), project.getId(), "task-status.created", response);
    return response;
  }
  ...
  public TaskStatusResponse updateStatus(Long statusId, UpdateTaskStatusRequest request) {
    ...
    if (request.getCode() != null) {
      ...
      if (!normalizedCode.equalsIgnoreCase(status.getCode())
          && taskStatusRepository.existsByProjectIdAndCodeIgnoreCase(
              status.getProject().getId(), normalizedCode)) {
        throw new ApplicationException(
            ErrorCode.INVALID_REQUEST_DATA, "Code status đã tồn tại trong project");
      }
    ...
    activityLogService.createLog(
        status.getProject().getWorkspace().getId(),
        currentUser.getUserId(),
        ActivityActionType.TASK_UPDATED,
        ActivityTargetType.STATUS,
        updatedStatus.getId(),
  ...
  @Override
  @Transactional
  public List<TaskStatusResponse> reorderStatuses(
      Long projectId, ReorderTaskStatusesRequest request) {
    ...
    if (statuses.size() != providedIds.size()) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Danh sách status không đầy đủ để reorder");
    }

    Set<Long> existingIds =
        statuses.stream().map(TaskStatus::getId).collect(HashSet::new, Set::add, Set::addAll);
    if (!existingIds.containsAll(providedIds)) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Danh sách status chứa ID không hợp lệ");
    }
    ...
    List<TaskStatusResponse> response =
        taskStatusRepository.findByProjectIdOrderByPositionAsc(projectId).stream()
            .map(taskStatusMapper::toResponse)
            .toList();

    realtimeEventPublisherService.publishProjectEvent(
        project.getWorkspace().getId(), project.getId(), "task-status.reordered", response);
  ...
  public void deleteStatus(Long statusId) {
    ...
    if (taskCount > 0) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Không thể xóa cột Kanban vì vẫn còn task");
    }
    ...
    List<TaskStatus> remainingStatuses =
        taskStatusRepository.findByProjectIdOrderByPositionAsc(project.getId());
    ...
    realtimeEventPublisherService.publishProjectEvent(
        workspaceId, project.getId(), "task-status.deleted", statusId);
  }
  ...
  private void moveStatusPosition(TaskStatus status, Integer targetPositionInput) {
    List<TaskStatus> statuses =
        taskStatusRepository.findByProjectIdOrderByPositionAsc(status.getProject().getId());
