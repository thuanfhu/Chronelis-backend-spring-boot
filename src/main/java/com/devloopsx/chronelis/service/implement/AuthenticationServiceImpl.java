...
public class AuthenticationServiceImpl implements AuthenticationService {
  ...
  public void register(RegisterUserRequest registerUserRequest) {
    ...
    String formattedPhoneNumber =
        this.phoneNumberUtils.formatPhoneNumberToE164(registerUserRequest.getPhoneNumber(), "VN");
  ...
  public void resendVerifyEmail(ResendVerifyEmailRequest resendVerifyEmailRequest) {
    User currentUser =
        this.userRepository
            .findByEmail(resendVerifyEmailRequest.getEmail())
            .orElseThrow(() -> new ApplicationException(ErrorCode.USER_NOT_FOUND));
    if (currentUser.getIsVerified())
      throw new ApplicationException(ErrorCode.NOT_VERIFIED_ACCOUNT_TWICE);
  ...
  public ResponseEntity<AuthenticationResponse> login(LoginRequest loginRequest) {
    ...
    if (emailLoginForm != null && !emailLoginForm.isEmpty()) {
      currentUser =
          this.userRepository
              .findByEmail(emailLoginForm)
              .orElseThrow(() -> new ApplicationException(ErrorCode.USER_NOT_FOUND));
    } else {
      String formattedPhoneNumber =
          this.phoneNumberUtils.formatPhoneNumberToE164(phoneNumberLoginForm, "VN");
      currentUser =
          this.userRepository
              .findByPhoneNumber(formattedPhoneNumber)
              .orElseThrow(() -> new ApplicationException(ErrorCode.USER_NOT_FOUND));
    }
    ...
    if (!isPasswordMatch) throw new ApplicationException(ErrorCode.UNAUTHENTICATED);
