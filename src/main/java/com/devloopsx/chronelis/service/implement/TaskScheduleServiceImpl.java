...
public class TaskScheduleServiceImpl implements TaskScheduleService {
    ...
    public TaskScheduleResponse createSchedule(CreateTaskScheduleRequest request) {
        ...
        if (task.getAssignee() != null && !task.getAssignee().getUserId().equals(currentUser.getUserId())) {
            notificationService.createAndPublish(task.getAssignee().getUserId(), NotificationType.TASK_RESCHEDULED,
                    "Task được lên lịch", "Task " + task.getTitle() + " vừa được lên lịch mới", ReferenceType.TASK,
                    task.getId());
        }
    ...
    public TaskScheduleResponse updateSchedule(Long scheduleId, UpdateTaskScheduleRequest request) {
        ...
        TaskSchedule schedule = taskScheduleRepository.findById(scheduleId)
                .orElseThrow(
                        () -> new ApplicationException(ErrorCode.RESOURCE_NOT_FOUND, "Task schedule không tồn tại"));
        ...
        activityLogService.createLog(task.getProject().getWorkspace().getId(), currentUser.getUserId(),
                ActivityActionType.TASK_RESCHEDULED, ActivityTargetType.SCHEDULE, updatedSchedule.getId(),
                "Cập nhật lịch cho task " + task.getTitle());

        if (task.getAssignee() != null && !task.getAssignee().getUserId().equals(currentUser.getUserId())) {
            notificationService.createAndPublish(task.getAssignee().getUserId(), NotificationType.TASK_RESCHEDULED,
                    "Task đổi lịch", "Task " + task.getTitle() + " vừa được cập nhật lịch", ReferenceType.TASK,
                    task.getId());
        }
