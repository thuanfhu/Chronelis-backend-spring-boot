...
public class CollaborationAccessServiceImpl implements CollaborationAccessService {
  ...
  public Workspace requireWorkspace(Long workspaceId) {
    return workspaceRepository
        ...
        .orElseThrow(
            () ->
                new ApplicationException(ErrorCode.RESOURCE_NOT_FOUND, "Workspace không tồn tại"));
  }

  @Override
  public WorkspaceMember requireWorkspaceMember(Long workspaceId, String userId) {
    return workspaceMemberRepository
        .findByWorkspaceIdAndUserUserId(workspaceId, userId)
        .orElseThrow(
            () ->
                new ApplicationException(
                    ErrorCode.UNAUTHORIZED_ACCESS, "Người dùng không thuộc workspace này"));
  }
  ...
  public void ensureCurrentUserIsWorkspaceOwner(Long workspaceId) {
    ...
    if (member.getRole() != com.devloopsx.chronelis.constant.WorkspaceMemberRoleType.OWNER) {
      throw new ApplicationException(
          ErrorCode.UNAUTHORIZED_ACCESS, "Chỉ owner workspace mới có quyền thực hiện thao tác này");
    }
  }
  ...
  public Project requireProject(Long projectId) {
    return projectRepository
        .findById(projectId)
        .orElseThrow(
            () -> new ApplicationException(ErrorCode.RESOURCE_NOT_FOUND, "Project không tồn tại"));
  }
  ...
  public Goal requireGoal(Long goalId) {
    return goalRepository
        .findById(goalId)
        .orElseThrow(
            () -> new ApplicationException(ErrorCode.RESOURCE_NOT_FOUND, "Goal không tồn tại"));
  }
  ...
  public TaskStatus requireTaskStatus(Long statusId) {
    return taskStatusRepository
        ...
        .orElseThrow(
            () ->
                new ApplicationException(
                    ErrorCode.RESOURCE_NOT_FOUND, "Task status không tồn tại"));
  }
  ...
  public WorkspaceTeam requireWorkspaceTeam(Long teamId) {
    return workspaceTeamRepository
        .findById(teamId)
        .orElseThrow(
            () -> new ApplicationException(ErrorCode.RESOURCE_NOT_FOUND, "Team không tồn tại"));
  }
  ...
  public void ensureCurrentUserCanAccessProject(Long projectId) {
    ...
    requireMinimumProjectRole(
        project, EffectiveProjectAccessRoleType.VIEWER, "Bạn không có quyền truy cập project này");
  }
  ...
  public void ensureCurrentUserCanManageProjectWork(Long projectId) {
    ...
    requireMinimumProjectRole(
        project,
        EffectiveProjectAccessRoleType.MANAGER,
        "Bạn không có quyền quản lý công việc trong project này");
  }

  @Override
  public void ensureCurrentUserCanManageProjectAccess(Long projectId) {
    EffectiveProjectAccessResponse access =
        projectPermissionService.resolveCurrentUserAccess(projectId);
  ...
  public void ensureCurrentUserCanDeleteProject(Long projectId) {
    ...
    if (!access.isCanDeleteProject()) {
      throw new ApplicationException(
          ErrorCode.UNAUTHORIZED_ACCESS, "Chỉ owner workspace mới có quyền xóa project");
    }
  }
  ...
  public void ensureCurrentUserCanAssignOthers(Long projectId) {
    ...
    if (!access.isCanAssignOthers()) {
      throw new ApplicationException(
          ErrorCode.UNAUTHORIZED_ACCESS, "Bạn không có quyền giao task cho người khác");
    }
  }
  ...
  public void ensureAssigneeBelongsWorkspace(String assigneeId, Long workspaceId) {
    ...
    boolean isMember =
        workspaceMemberRepository.existsByWorkspaceIdAndUserUserId(workspaceId, assigneeId);
    if (!isMember) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Người được giao việc không thuộc workspace");
    }
  }

  private void requireMinimumProjectRole(
      Project project, EffectiveProjectAccessRoleType requiredRole, String message) {
    EffectiveProjectAccessRoleType effectiveRole =
        projectPermissionService.resolveCurrentUserRole(project);
