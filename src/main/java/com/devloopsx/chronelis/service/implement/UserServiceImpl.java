...
public class UserServiceImpl implements UserService {
  ...
  @Override
  public UserSecureResponse updateUserPassword(
      UpdateUserPasswordRequest updateUserPasswordRequest) {
    User userAuthenticated = this.securityUtils.getAuthenticatedUser();

    if (!passwordEncoder.matches(
        updateUserPasswordRequest.getCurrentPassword(), userAuthenticated.getPassword()))
      throw new ApplicationException(ErrorCode.PASSWORD_MISMATCH);
    if (!updateUserPasswordRequest
        .getNewPassword()
        .equals(updateUserPasswordRequest.getConfirmPassword()))
      throw new ApplicationException(ErrorCode.PASSWORD_AND_CONFIRM_MISMATCH);
    if (passwordEncoder.matches(
        updateUserPasswordRequest.getNewPassword(), userAuthenticated.getPassword()))
    ...
    UserSecureResponse response =
        this.userMapper.userToSecureResponse(this.userRepository.save(userAuthenticated));
  ...
  public void updateUserEmail(UpdateUserEmailRequest updateUserEmailRequest) {
    ...
    securityUtils.enforceProtectedEmailPolicy(
        userAuthenticated.getEmail()); // can't change system email
  ...
  public UserSecureResponse verifyEmailAndChangeNewEmail(VerifyEmailRequest verifyEmailRequest)
    ...
    UserSecureResponse response =
        userMapper.userToSecureResponse(this.userRepository.save(userAuthenticated));
    ...
    return PaginationResponse.builder()
        .meta(
            PaginationMeta.builder()
                ...
                .build())
        .content(
            this.securityUtils.isAdmin(userAuthenticated)
  ...
  @Override
  public UserResponse updateUserForAdmin(
    ...
    if (currentPhoneNumber != null && !currentPhoneNumber.isEmpty()) {
      String formattedPhoneNumber =
          phoneNumberUtils.formatPhoneNumberToE164(currentPhoneNumber, "VN");
  ...
  public void deleteUserById(String userId) {
    ...
    User replacementUser =
        userRepository
            .findById(securityUtils.getAuthenticatedUser().getUserId())
            .orElseThrow(() -> new ApplicationException(ErrorCode.USER_NOT_FOUND));

    if (targetUser.getUserId().equals(replacementUser.getUserId())) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Không thể tự xóa tài khoản đang đăng nhập");
    }
  ...
  public void deleteRoleFromUser(
      String userId, DeleteRoleFromUserRequest deleteRoleFromUserRequest) {
    User currentUser =
        userRepository
            .findById(userId)
            .orElseThrow(() -> new ApplicationException(ErrorCode.USER_NOT_FOUND));
    ...
    if (!nonExistentInUser.isEmpty()) {
      throw new ApplicationException(
          ErrorCode.ROLE_NOT_IN_USER,
          "Vai trò với ID: "
              + nonExistentInUser
              + " không có trong người dùng "
