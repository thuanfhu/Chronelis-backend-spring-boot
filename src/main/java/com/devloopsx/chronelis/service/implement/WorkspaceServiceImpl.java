...
public class WorkspaceServiceImpl implements WorkspaceService {
  ...
  public WorkspaceResponse updateWorkspace(Long workspaceId, UpdateWorkspaceRequest request) {
    ...
    activityLogService.createLog(
        workspaceId,
        currentUser.getUserId(),
        ActivityActionType.WORKSPACE_UPDATED,
        ActivityTargetType.WORKSPACE,
        workspaceId,
  ...
  public PaginationResponse listVisibleWorkspaces(Pageable pageable) {
    User currentUser = securityUtils.getAuthenticatedUser();
    Page<Workspace> page =
        workspaceRepository.findVisibleByUserId(currentUser.getUserId(), pageable);
  ...
  public WorkspaceMemberResponse addMember(Long workspaceId, AddWorkspaceMemberRequest request) {
    ...
    if (workspaceMemberRepository.existsByWorkspaceIdAndUserUserId(
        workspaceId, targetUser.getUserId())) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Người dùng đã là thành viên workspace");
    }
    ...
    notificationService.createAndPublish(
        ...
        "Bạn được thêm vào workspace",
        "Bạn vừa được thêm vào workspace " + workspace.getName(),
  ...
  public WorkspaceMemberResponse updateMemberRole(
            ...
            .orElseThrow(
                () ->
                    new ApplicationException(
                        ErrorCode.RESOURCE_NOT_FOUND, "Thành viên workspace không tồn tại"));
    ...
    if (member.getRole() == WorkspaceMemberRoleType.OWNER
        && request.getRole() != WorkspaceMemberRoleType.OWNER) {
      long ownerCount =
          workspaceMemberRepository.countByWorkspaceIdAndRole(
              workspaceId, WorkspaceMemberRoleType.OWNER);
      if (ownerCount <= 1) {
        throw new ApplicationException(
            ErrorCode.INVALID_REQUEST_DATA, "Không thể hạ cấp OWNER duy nhất của workspace");
      }
    }
  ...
  public void removeMember(Long workspaceId, String userId) {
    ...
    if (member.getRole() == WorkspaceMemberRoleType.OWNER) {
      ...
      if (ownerCount <= 1) {
        throw new ApplicationException(
            ErrorCode.INVALID_REQUEST_DATA, "Không thể xóa OWNER duy nhất của workspace");
      }
    }
    ...
    notificationService.createAndPublish(
        ...
        "Bạn đã bị xóa khỏi workspace " + workspace.getName(),
        ReferenceType.WORKSPACE,
        workspaceId);
  }
  ...
  private User resolveWorkspaceMemberTarget(String userIdentifier) {
    ...
    if (normalizedIdentifier.isEmpty()) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Thiếu thông tin định danh người dùng");
    }
  ...
  private void publishWorkspaceEventAfterCommit(Long workspaceId, String eventType, Object data) {
    afterCommitExecutor.runAfterCommit(
        () -> realtimeEventPublisherService.publishWorkspaceEvent(workspaceId, eventType, data));
  }
}
