...
public class WorkspaceInviteServiceImpl implements WorkspaceInviteService {
  ...
  public WorkspaceInviteResponse createInvite(CreateWorkspaceInviteRequest request) {
    ...
    activityLogService.createLog(
        ...
        saved.getId(),
        "Tạo lời mời tham gia workspace " + workspace.getName());

    WorkspaceInviteResponse response = workspaceInviteMapper.toResponse(saved);
    realtimeEventPublisherService.publishWorkspaceEvent(
        workspace.getId(), "invite.created", response);
    return response;
  }
  ...
  public List<WorkspaceInviteResponse> listActiveInvites(Long workspaceId) {
    ...
    return workspaceInviteRepository
        ...
        .stream()
        .map(workspaceInviteMapper::toResponse)
        .toList();
  }
  ...
  public void revokeInvite(Long inviteId) {
            ...
            .orElseThrow(
                () ->
                    new ApplicationException(
                        ErrorCode.RESOURCE_NOT_FOUND, "Lời mời không tồn tại"));
    ...
    activityLogService.createLog(
        ...
        ActivityTargetType.INVITE,
        inviteId,
        "Thu hồi lời mời workspace " + invite.getWorkspace().getName());

    realtimeEventPublisherService.publishWorkspaceEvent(
        invite.getWorkspace().getId(), "invite.revoked", inviteId);
  }
    ...
    WorkspaceInvite invite =
        workspaceInviteRepository
            ...
            .orElseThrow(
                () ->
                    new ApplicationException(
                        ErrorCode.RESOURCE_NOT_FOUND, "Mã mời không hợp lệ hoặc đã hết hạn"));
  ...
  public void joinByInvite(JoinByInviteRequest request) {
    ...
    if (workspaceMemberRepository.existsByWorkspaceIdAndUserUserId(
        workspaceId, currentUser.getUserId())) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Bạn đã là thành viên của workspace này");
    }
