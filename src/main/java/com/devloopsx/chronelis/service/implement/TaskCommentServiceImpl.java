...
public class TaskCommentServiceImpl implements TaskCommentService {
              ...
              .orElseThrow(
                  () ->
                      new ApplicationException(
                          ErrorCode.RESOURCE_NOT_FOUND, "Comment cha không tồn tại"));

      if (!parentComment.getTask().getId().equals(task.getId())) {
        throw new ApplicationException(
            ErrorCode.INVALID_REQUEST_DATA, "Comment cha không thuộc task được chọn");
      }

      if (parentComment.getParentComment() != null) {
        throw new ApplicationException(
            ErrorCode.INVALID_REQUEST_DATA, "Chỉ hỗ trợ trả lời một cấp");
      }
  ...
  public TaskCommentResponse updateComment(Long commentId, UpdateTaskCommentRequest request) {
    TaskComment comment =
        taskCommentRepository
            ...
            .orElseThrow(
                () ->
                    new ApplicationException(
                        ErrorCode.RESOURCE_NOT_FOUND, "Comment không tồn tại"));
    ...
    if (!canModifyComment(comment, currentUser)) {
      throw new ApplicationException(
          ErrorCode.UNAUTHORIZED_ACCESS, "Bạn không có quyền sửa bình luận này");
    }
  ...
  public void deleteComment(Long commentId) {
    ...
    if (!canModifyComment(comment, currentUser)) {
      throw new ApplicationException(
          ErrorCode.UNAUTHORIZED_ACCESS, "Bạn không có quyền xóa bình luận này");
    }
  ...
  private boolean canModifyComment(TaskComment comment, User currentUser) {
    ...
    return projectPermissionService
        .resolveCurrentUserRole(comment.getTask().getProject())
  ...
  private void notifyCommentParticipants(Task task, User actor, Long commentId) {
    ...
    for (String recipientId : recipients) {
      notificationService.createAndPublish(
          ...
          "Có bình luận mới",
          actor.getEmail() + " vừa bình luận task " + task.getTitle(),
