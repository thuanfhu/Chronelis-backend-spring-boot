...
public class TaskServiceImpl implements TaskService {
  ...
  public TaskResponse createTask(CreateTaskRequest request) {
    ...
    if (!status.getProject().getId().equals(project.getId())) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Task status không thuộc project được chọn");
    }
    ...
    if (request.getGoalId() != null) {
      ...
      if (!goal.getProject().getId().equals(project.getId())) {
        throw new ApplicationException(
            ErrorCode.INVALID_REQUEST_DATA, "Goal không thuộc project của task");
      }
    }
    ...
    if (request.getAssigneeId() != null && !request.getAssigneeId().isBlank()) {
      ...
      collaborationAccessService.ensureAssigneeBelongsWorkspace(
          request.getAssigneeId(), project.getWorkspace().getId());
    ...
    task.setEstimatedMinutes(
        request.getEstimatedMinutes() == null ? 0 : request.getEstimatedMinutes());
    task.setSourceView(
        request.getSourceView() != null ? request.getSourceView() : SourceViewType.KANBAN);

    if (request.getTaskTypeId() != null) {
      TaskType taskType =
          taskTypeRepository
              .findById(request.getTaskTypeId())
              .orElseThrow(
                  () ->
      ...
      if (!taskType.getProject().getId().equals(project.getId())) {
        throw new ApplicationException(
            ErrorCode.INVALID_REQUEST_DATA, "Task type không thuộc project được chọn");
      }
    ...
    if (task.getEstimatedMinutes() < 0) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "estimated_minutes không được âm");
    }
    ...
    if (assignee != null && !assignee.getUserId().equals(currentUser.getUserId())) {
      notificationService.createAndPublish(
          ...
          "Bạn được giao task",
          "Bạn vừa được giao task: " + savedTask.getTitle(),
          ReferenceType.TASK,
          savedTask.getId());
    }
  ...
  public TaskResponse updateTask(Long taskId, UpdateTaskRequest request) {
    ...
    if (request.getTitle() == null
        ...
        && !Boolean.TRUE.equals(request.getClearGoal())
        && request.getPriority() == null
        && request.getDueDate() == null
        && request.getEstimatedMinutes() == null
        && request.getTaskTypeId() == null
        && request.getNotesHtml() == null) {
    ...
    if (request.getTaskTypeId() != null) {
      ...
      if (!taskType.getProject().getId().equals(task.getProject().getId())) {
        throw new ApplicationException(
            ErrorCode.INVALID_REQUEST_DATA, "Task type không thuộc project của task");
      }
      task.setTaskType(taskType);
    }
  ...
  public TaskResponse moveTask(Long taskId, MoveTaskRequest request) {
    ...
    if (!targetStatus.getProject().getId().equals(task.getProject().getId())) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Task status không thuộc project của task");
    }
    ...
    if (!sourceStatusId.equals(targetStatus.getId())) {
      ...
      if (updatedTask.getAssignee() != null
        ...
        notificationService.createAndPublish(
            updatedTask.getAssignee().getUserId(),
    ...
    return reorderTask(
        taskId, ReorderTaskRequest.builder().targetPosition(request.getTargetPosition()).build());
  }
  ...
  public TaskResponse reorderTask(Long taskId, ReorderTaskRequest request) {
    ...
    List<Task> tasks =
        new ArrayList<>(taskRepository.findByStatusIdOrderByBoardPositionAscIdAsc(statusId));
    ...
    Task updatedTask =
        taskRepository
            .findById(taskId)
            .orElseThrow(
                () -> new ApplicationException(ErrorCode.RESOURCE_NOT_FOUND, "Task không tồn tại"));

    User currentUser = securityUtils.getAuthenticatedUser();
    activityLogService.createLog(
        task.getProject().getWorkspace().getId(),
        currentUser.getUserId(),
        ActivityActionType.TASK_REORDERED,
        ActivityTargetType.TASK,
        taskId,
  ...
  public TaskResponse assignTask(Long taskId, AssignTaskRequest request) {
    ...
    if (request.getAssigneeId() == null || request.getAssigneeId().isBlank()) {
      ...
      TaskResponse response = toTaskResponse(updatedTask);
      invalidateTaskMutationAfterCommit(
          task.getProject().getId(), oldAssigneeId == null ? Set.of() : Set.of(oldAssigneeId));
      publishTaskEventAfterCommit(
          task.getProject().getWorkspace().getId(),
          task.getProject().getId(),
          task.getId(),
    ...
    collaborationAccessService.ensureAssigneeBelongsWorkspace(
        request.getAssigneeId(), task.getProject().getWorkspace().getId());
    ...
    if (!newAssignee.getUserId().equals(currentUser.getUserId())
      ...
      notificationService.createAndPublish(
          ...
          NotificationType.TASK_ASSIGNED,
          "Bạn được giao task",
          "Bạn vừa được giao task: " + updatedTask.getTitle(),
          ReferenceType.TASK,
          updatedTask.getId());
    }
  ...
  public MyWorkResponse getMyWork() {
    ...
    return redisCacheService
        .getJson(key, MyWorkResponse.class)
        .orElseGet(
            () -> {
  ...
  private MyWorkResponse buildMyWork(User currentUser) {
    List<Long> workspaceIds =
        workspaceMemberRepository.findByUserUserId(currentUser.getUserId()).stream()
    ...
    List<Task> assignedTasks =
        taskRepository.findVisibleAssignedOpenTasks(
            currentUser.getUserId(), workspaceIds, PageRequest.of(0, 500));
    ...
    List<MyWorkScheduleItemResponse> upcomingSchedules =
            ...
            .map(
                schedule -> {
                  TaskResponse taskResponse =
                      taskResponseById.computeIfAbsent(
  ...
  public TaskAnalyticsResponse getTaskAnalytics() {
    ...
    return redisCacheService
        .getJson(key, TaskAnalyticsResponse.class)
        .orElseGet(
            () -> {
  ...
  private TaskAnalyticsResponse buildTaskAnalytics(User currentUser) {
    ...
    for (int i = 13; i >= 0; i--) {
      ...
      trend.add(
          TaskAnalyticsResponse.DailyTrendPoint.builder()
              .date(date)
    ...
    for (Object[] row : taskRepository.sumEstimatedByPriorityForUser(userId, workspaceIds)) {
      estimates.add(
          TaskAnalyticsResponse.PriorityEstimatePoint.builder()
  ...
  public void deleteTask(Long taskId) {
    ...
    invalidateTaskMutationAfterCommit(
        projectId, assigneeId == null ? Set.of() : Set.of(assigneeId));
  ...
  private TaskStatus resolveFirstClosedStatus(Long projectId) {
    return taskStatusRepository
        .findFirstByProjectIdAndIsClosedTrueOrderByPositionAsc(projectId)
        .orElseThrow(
            () ->
                new ApplicationException(
                    ErrorCode.INVALID_REQUEST_DATA,
  ...
  private TaskStatus resolveRestoreOpenStatus(Task task) {
    ...
    if (task.getLastOpenStatus() != null) {
      Optional<TaskStatus> lastOpenStatus =
          taskStatusRepository.findByProjectIdAndIdAndIsClosedFalse(
              projectId, task.getLastOpenStatus().getId());
    ...
    return taskStatusRepository
        .findFirstByProjectIdAndIsClosedFalseOrderByPositionAsc(projectId)
        .orElseThrow(
            () ->
                new ApplicationException(
                    ErrorCode.INVALID_REQUEST_DATA, "Project chưa có cột mở để khôi phục task"));
  }
  ...
  private void shiftRightFromPosition(Long statusId, int fromPosition) {
    List<Task> toShift =
        taskRepository.findByStatusIdAndBoardPositionGreaterThanEqualOrderByBoardPositionAscIdAsc(
            statusId, fromPosition);
  ...
  private void shiftLeftAfterRemoval(Long statusId, int removedPosition) {
    List<Task> toShift =
        taskRepository.findByStatusIdAndBoardPositionGreaterThanOrderByBoardPositionAscIdAsc(
            statusId, removedPosition);
    for (Task item : toShift) {
  ...
  private TaskResponse toTaskResponse(Task task) {
    ...
    applyDependencySummary(
        ...
        taskDependencyService
            .summarizeTasks(List.of(task.getId()), blockerNotesByTaskId(List.of(task)))
            .get(task.getId()));
    return response;
  }
  ...
  private List<TaskResponse> toTaskResponses(List<Task> tasks) {
    ...
    List<TaskResponse> responses =
        new ArrayList<>(tasks.stream().map(taskMapper::toResponse).toList());
  ...
  private void applyDependencySummary(
      TaskResponse response, TaskDependencyService.TaskDependencySummary dependencySummary) {
  ...
  private void invalidateTaskMutationAfterCommit(
      Long projectId, Collection<String> affectedUserIds) {
  ...
  private void ensureAssigneeCanAccessProject(Project project, String assigneeId) {
    if (!projectPermissionService
        .resolveUserRole(project, assigneeId)
        .atLeast(EffectiveProjectAccessRoleType.VIEWER)) {
      throw new ApplicationException(
          ErrorCode.INVALID_REQUEST_DATA, "Assignee must have project access");
    }
  }
}
