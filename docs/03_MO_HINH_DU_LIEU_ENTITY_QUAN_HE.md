...
3. Nhóm thực thi công việc: `Project`, `Goal`, `TaskStatus`, `TaskType`, `Task`, `TaskSchedule`, `TaskComment`.
4. Nhóm theo dõi sự kiện: `Notification`, `ActivityLog`.
...
| `WorkspaceMember`     | thành viên workspace   | `workspace`, `user`, `role`, `joinedAt`                                                                 | role là `OWNER/ADMIN/MEMBER`            |
...
| `WorkspaceInvite`     | mã mời vào workspace   | `inviteCode`, `roleToAssign`, `maxUses`, `usedCount`, `expiresAt`, `isActive`                           | không cho assign `OWNER`                |
| `Project`             | dự án trong workspace  | `workspace`, `name`, `description`, `status`, `managerUser`, `managerTeam`                              | có manager theo user hoặc team          |
...
`Goal` cũng có mô hình tương tự project. Điều này cho phép phân rã quyền quản lý theo cấp mục tiêu mà không cần owner/admin thao tác mọi task.
...
3. Invite không được gán `OWNER`.
4. Member mới không được thêm trực tiếp với role `OWNER`.
